...
uthread_init(void)
{
  // initialize all queues in run queues
  init_runqueues();
...
uthread_create(uthread_id_t *uidp, uthread_func_t func,
  ...
  uthread_id_t uid = uthread_alloc();

  // if no valid id found return -1
  if(uid < 0)
    return -1;
  ...
  // if malloc call in alloc_stack() returns NULL
  if(!stack)
    return -1;

  uthread_makecontext(&uthreads[uid].ut_ctx, stack, UTH_STACK_SIZE, func, arg1, arg2);

  // update thread struct in array (only fill in certain elements)
  uthreads[uid].ut_state = UT_RUNNABLE;

  // set priority (make runnable)
  uthread_setprio(uid, prio);
...
uthread_exit(int status)
{
  ...
  // else wake up waiting thread
  if(!(ut_curthr->ut_detached) && ut_curthr->ut_waiter) {
    uthread_wake(ut_curthr->ut_waiter);
  }
  else if (ut_curthr->ut_detached) {
    // if thread is detached, call reaper
...
uthread_join(uthread_id_t uid, int *return_value)
{
  ...
  if(!uthreads[uid].ut_state)
    return ESRCH;

  // another thread already waiting to join with uid
  if(uthreads[uid].ut_waiter)
    return EINVAL;

  // if the thread has not exited, block
  if(uthreads[uid].ut_state != UT_ZOMBIE) {
    // set the current thread as waiting on uid
    uthreads[uid].ut_waiter = ut_curthr;
    uthread_block();
  }

  // place exit value of thread into return value
  *return_value = uthreads[uid].ut_exit;

  // once thread has become a zombie reap it
...
uthread_detach(uthread_id_t uid)
{
  ...
  // if you call detach on an invalid thread i.e. one that's already detached
  ...
  // calling detach on thread that has already finished executing
  if(uthreads[uid].ut_state == UT_ZOMBIE) {
    make_reapable(&uthreads[uid]);
  }
...
uthread_alloc(void)
{
  ...
  // number threads [0, UTH_MAX_UTHREADS)
  for( ; i < UTH_MAX_UTHREADS; i++) {
    if(!uthreads[i].ut_state) {
      uthreads[i].ut_id = i;
...
uthread_destroy(uthread_t *uth)
{
  ...
  uth->ut_prio = RESET;
  uth->ut_errno = RESET;
  uth->ut_has_exited = false;
  uth->ut_exit = RESET;
  uth->ut_detached = false;
  uth->ut_waiter = NULL;
}
