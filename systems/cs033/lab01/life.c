...
void update(int rows, int cols, int old_array[rows * cols], int new_array[rows * cols]) {
    ...
    {
	// translate i to current position
	int r = (i/cols);
	int c = (i%cols);

        int state = get_next_state(rows, cols, old_array, r, c);

	if(state)
	    set_alive(rows, cols, new_array, r, c);
	else
	    set_dead(rows, cols, new_array, r, c);
    }
}
...
int get_next_state(int rows, int cols, int array[rows * cols], int row, int col) {
    ...
    int alive = is_alive(rows, cols, array, row, col);
...
int count_alive_neighbors(int rows, int cols, int array[rows * cols], int row, int col) {
    ...
    int row_pos[] = {(row - 1), row, (row + 1)};
    int col_pos[] = {(col - 1), col, (col + 1)};
    int pos_len = 3;
    ...
    for(int i = 0; i < pos_len; i++)
    {
        ...
        {
	    if(row_pos[i] == row && col_pos[k] == col)
		continue;
	    else
	        number_alive = number_alive + is_alive(rows, cols, array, row_pos[i], col_pos[k]);
        }
    }
...
void print_array(int rows, int cols, int array[rows * cols]) {
    ...
    for(int i = 0; i < rows; i++)
    {
        ...
        {
            printf("%i ", is_alive(rows, cols, array, i, k));
        }
