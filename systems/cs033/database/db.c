...
node_t head = {"", "", 0, 0, .lock = PTHREAD_RWLOCK_INITIALIZER};
...
void db_query(char *name, char *result, int len) {
    ...
    int error;
    if((error = pthread_rwlock_wrlock(&head.lock))) {
      char buf[MAXLEN] ;
      int num_chars = sprintf(buf, "locking head: %i\n", error);
      write(STDERR_FILENO, buf, num_chars);
      exit(1);
    }

    target = search(name, &head, 0, lt);
    if (target == 0) {
...
int db_add(char *name, char *value) {
    ...
    // if node is already in the tree, unlock everything and return
    if ((target = search(name, &head, &parent, lt)) != 0) {
	pthread_rwlock_unlock(&target->lock);
...
int db_remove(char *name) {
    ...
    if ((dnode = search(name, &head, &parent, lt)) == 0) {
        // it's not there
	pthread_rwlock_unlock(&parent->lock);
        return(0);
    }
