...
typedef struct write_data {
    size_t wpos;		/* position of a particular write in the buffer */
    size_t wsz;			/* size of a particular write in the buffer */
} write_data;
...
//    n is the number of writes. this is used primarily to help with reordercat61.c

void insert_into_wbuf(io61_file* f, size_t pos, size_t sz) {
...
/* big difference here: assume there is a field in f->wbuf that
 * holds the index of the last element in wbuf: 'wsz'. Let's use
 * wpos for ordering purposes */
void bin_insert_into_wbuf(io61_file* f, size_t pos, size_t sz, size_t first, size_t last) {
  size_t index = (first + last) / 2;

  /* base case; this will increase the number of elements in the wbuf */
  if(index >= last) {
    f->wbuf[f->wsz].wsz = sz;
    f->wbuf[f->wsz].wpos = pos;
    f->wsz++;
    return;
  }
  ...
  if(pos >= mpos) {
    ...
    if(pos == (mpos + msz)) {
      ...
      bin_insert_into_wbuf(f, pos, sz, index + 1, last);
    }
  } else {
    /* current write will end within a previous write */
    if((pos + sz >= mpos) && (pos + sz <= mpos + msz)) {
      ...
      bin_insert_into_wbuf(f, pos, sz, first, index - 1);
    }
  }
}
...
int io61_writec(io61_file* f, int ch) {
    ...
    if(!(BLOCK ^ f->pos)) {
      ...
      bin_insert_into_wbuf(f, f->pos, 1, 0, f->wsz);
      f->buf[f->pos++] = ch;
    }
    else {
      bin_insert_into_wbuf(f, f->pos, 1, 0, f->wsz);
...
ssize_t io61_write(io61_file* f, const char* buf, size_t sz) {
    ...
    if(sz <= (BLOCK - f->pos)) {
      ...
      bin_insert_into_wbuf(f, f->pos, sz, 0, f->wsz);
      f->pos += sz;
...
int io61_seek(io61_file* f, off_t pos) {
    ...
    if (r == (off_t) pos) {
	   ...
	   * in io61_read() */
	  lseek(f->fd, pos + (pos - f->abs + f->sz), SEEK_SET);
	} else {
	  f->pos = 0;
	  f->sz = 0;
	  f->abs = pos;
