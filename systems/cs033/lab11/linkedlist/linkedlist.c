...
list_ele_t *search(int val, list_ele_t **parentp) {
    ...
    // lock the current element and the next element
...
void deleteList(int val) {
    ...
    pthread_mutex_unlock(&oldItem->mutex);
    pthread_mutex_destroy(&oldItem->mutex);
    free(oldItem);
