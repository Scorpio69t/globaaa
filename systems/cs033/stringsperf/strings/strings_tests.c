...
void test_strncpy() {
    char abbreviation[5];
    strncpy(abbreviation, "professor", 10);
...
void test_strspn() {
    ...
    assert_equal_int(12, strspn(s, accept),
	"strspn(\"we all love strings\", \"we alove\")");
}
...
void test_strcspn() {
    ...
    assert_equal_int(3, strcspn(s, reject),
	"strcspn(\"coming up with example strings gets hard\", \"breakingthe4thwall\")");
}
...
void test_strncmp() {
    ...
    assert_equal_int(0, strncmp("same", "samexyz", 4),
        "strstr(\"same\", \"samexyz\", \"4\")");
}
...
void test_strstr() {
    ...
    char* delim = ",";
    assert_equal_str(",2,,,3,4", strstr("1,2,,,3,4", delim), 
         "strstr(\"1,2,,,3,4\", \",\")");
}
...
void time_test(void (*func)(), int num_tests, const char *message) {
    ...
    waitpid(pid, &status, 0);
    if (status != 0) 
        printf("%s (repeated %d times) (process exited with code: %d)\n", 
            message, num_repeat, status);
